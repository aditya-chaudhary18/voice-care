...
import { Bell, MessageSquare, Mail, Smartphone, User, Shield, Save, UserPlus, Stethoscope, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Manage your hospital profile, staff and preferences</p>
      </div>
          ...
          {activeTab === 'doctors' && (
            <div className="p-6">
              ...
              <div className="bg-muted/30 p-4 rounded-lg mb-8 border border-border/50">
                ...
                <form onSubmit={handleAddDoctor} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <input 
                    placeholder="Name" 
                    required 
              ...
              <div className="space-y-4">
                <h3 className="text-md font-medium text-muted-foreground">Registered Doctors ({doctors.length})</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {doctors.map(doctor => (
                    <div key={doctor.id} className="p-4 border border-border rounded-lg flex justify-between items-start bg-background/50">
                      <div>
                        <p className="font-semibold text-primary">{doctor.name}</p>
                        <p className="text-xs text-muted-foreground italic mb-2">{doctor.specialty || 'General Practitioner'}</p>
                        <div className="flex flex-col gap-1">
                          <span className="text-xs flex items-center gap-1.5 text-muted-foreground"><Mail className="w-3 h-3"/> {doctor.email}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                  {doctors.length === 0 && (
                    <div className="col-span-2 text-center py-8 text-muted-foreground bg-muted/20 rounded-lg border border-dashed border-border">
                      No doctors added yet. Use the form above to add your medical staff.
                    </div>
                  )}
                </div>
              </div>
